...
import com.macro.mall.portal.component.KafkaPayMessageSender;
import com.macro.mall.portal.component.PayFailMessageSender;
...
public class AlipayServiceImpl implements AlipayService {
    ...
    public String notify(Map<String, String> params) {
        ...
        if (signVerified) {
            ...
            if("TRADE_SUCCESS".equals(tradeStatus)){
                result = "success";
                log.info("支付回调校验成功，tradeStatus: {}", tradeStatus);
                
                // 构建支付消息
                PaySuccessMessage message = new PaySuccessMessage();
                message.setOrderSn(params.get("out_trade_no"));
                message.setPayAmount(new BigDecimal(params.get("total_amount")));
                message.setPayType(1); // 支付宝
                message.setTradeNo(params.get("trade_no"));
                message.setPayTime(new Date());
                
                // 发送Kafka消息（解耦关键）
                kafkaPayMessageSender.sendPaySuccessMessage(message);
            } else if ("TRADE_CLOSED".equals(tradeStatus)) {
                // 支付关闭/未付款超时关闭
                log.warn("订单支付关闭: orderSn={}, tradeStatus={}", 
                        params.get("out_trade_no"), tradeStatus);
                sendPayFailMessage(params.get("out_trade_no"), 1, "TRADE_CLOSED", true);
            } else {
                log.warn("订单支付未成功: orderSn={}, tradeStatus={}", 
                        params.get("out_trade_no"), tradeStatus);
                sendPayFailMessage(params.get("out_trade_no"), 1, tradeStatus, true);
            }
        } else {
            log.warn("支付回调签名校验失败！");
            // 签名失败也需要释放库存
            sendPayFailMessage(params.get("out_trade_no"), 1, "SIGN_FAILED", true);
        }
        return result;
    }
     ...
     * 发送支付失败消息
     */
    private void sendPayFailMessage(String orderSn, Integer payType, String failReason, Boolean cancelOrder) {
        ...
        // lockToken需要从订单中获取，这里简化处理
        // 实际项目中应该根据orderSn查询订单获取lockToken
        payFailMessageSender.sendPayFailMessage(message);
    }
    ...
    public String query(String outTradeNo, String tradeNo) {
        ...
        if(response.isSuccess()){
            ...
            if("TRADE_SUCCESS".equals(response.getTradeStatus())){
                ...
                try {
                    message.setPayAmount(new java.math.BigDecimal(response.getTotalAmount()));
                } catch (Exception e) {
                    message.setPayAmount(null);
                }
